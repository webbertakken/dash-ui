...
function ControlledInput<T>({ initial, render: renderProp }: { initial: T; render: (v: T, set: (n: T) => void) => React.ReactNode }) {
...
describe('ActionMenu', () => {
  it('opens, navigates with keyboard, activates, closes on Escape', () => {
    ...
    render(<U.ActionMenu items={[{ id: 'a', label: 'A' }, { id: 'b', label: 'B' }, { id: 'c', label: 'C', disabled: true }]} onAction={onAction} />);
...
describe('Combobox', () => {
  it('opens on focus, types, navigates, picks', () => {
    ...
    render(<U.Combobox label="Pick" options={[{ value: 'a', label: 'Apple' }, { value: 'b', label: 'Banana' }]} value="a" onChange={onChange} />);
...
describe('MultiSelect', () => {
  it('toggles options, selects all, clears all', () => {
    const onChange = vi.fn();
    const { container } = render(<U.MultiSelect options={[{ value: 'a', label: 'A' }, { value: 'b', label: 'B' }]} value={['a']} onChange={onChange} label="Pick" />);
...
describe('Select', () => {
  it('opens, navigates, picks', () => {
    const onChange = vi.fn();
    render(<U.Select options={[{ value: 'a', label: 'A' }, { value: 'b', label: 'B' }]} value="a" onChange={onChange} label="L" />);
    const trigger = screen.getByRole('combobox');
    fireEvent.click(trigger);
    fireEvent.keyDown(trigger, { key: 'ArrowDown' });
    fireEvent.keyDown(trigger, { key: 'Home' });
...
describe('SegmentedControl', () => {
  it('controlled selection by clicking', () => {
    const onChange = vi.fn();
    render(<U.SegmentedControl label="L" options={[{ value: 'a', label: 'A' }, { value: 'b', label: 'B' }]} value="a" onChange={onChange} />);
    fireEvent.click(screen.getByRole('radio', { name: 'B' }));
    expect(onChange).toHaveBeenCalledWith('b');
  });

  it('uncontrolled defaultValue switches', () => {
    render(<U.SegmentedControl label="L" options={[{ value: 'a', label: 'A' }, { value: 'b', label: 'B' }]} defaultValue="a" />);
    fireEvent.click(screen.getByRole('radio', { name: 'B' }));
  });

  it('disabled options not clickable', () => {
    render(<U.SegmentedControl label="L" options={[{ value: 'a', label: 'A' }, { value: 'b', label: 'B', disabled: true }]} />);
    fireEvent.click(screen.getByRole('radio', { name: 'B' }));
  });
});
...
describe('CommandPalette', () => {
  it('filters, navigates, picks, closes', () => {
    ...
    const onClose = vi.fn();
    render(<U.CommandPalette open items={[{ id: 'a', label: 'Open A', group: 'Pages' }, { id: 'b', label: 'Open B', shortcut: 'B' }]} onSelect={onSelect} onClose={onClose} />);
  ...
  it('clicking option picks', () => {
    const onSelect = vi.fn();
    render(<U.CommandPalette open items={[{ id: 'a', label: 'A' }]} onSelect={onSelect} onClose={() => {}} />);
    const opt = screen.getAllByRole('option')[0]!;
    fireEvent.mouseDown(opt);
  });
});
...
describe('Modal', () => {
  it('renders open, escape closes, traps focus', () => {
    const onClose = vi.fn();
    const { rerender } = render(<U.Modal open title="T" onClose={onClose}><button>inside</button></U.Modal>);
  ...
  it('backdrop mousedown + mouseup closes', () => {
    const onClose = vi.fn();
    const { container } = render(<U.Modal open title="T" onClose={onClose}><div>x</div></U.Modal>);
    const backdrop = container.querySelector('[class*=backdrop],.modal-backdrop,[role=dialog]')?.parentElement ?? container.firstElementChild!;
...
describe('Drawer', () => {
  it('escape + backdrop click + close button', () => {
    const onClose = vi.fn();
    const { rerender } = render(<U.Drawer open title="D" onClose={onClose}><button>inside</button></U.Drawer>);
...
describe('ContextMenu', () => {
  it('action + escape + outside close + disabled item ignored', () => {
    const onClose = vi.fn();
    const onAction = vi.fn();
    const { container } = render(<U.ContextMenu open x={10} y={10} onClose={onClose} onAction={onAction} items={[{ id: 'a', label: 'A' }, { id: 'b', label: 'B', disabled: true }]} />);
...
describe('Menubar', () => {
  it('opens menus and triggers actions', () => {
    const onAction = vi.fn();
    const { container } = render(<U.Menubar onAction={onAction} menus={[{ id: 'f', label: 'File', items: [{ id: 'open', label: 'Open' }, { id: 'sep', label: '', separator: true }, { id: 'close', label: 'Close', disabled: true }] }, { id: 'e', label: 'Edit', items: [{ id: 'cut', label: 'Cut' }] }]} />);
...
describe('Tabs', () => {
  it('controlled change + uncontrolled noop without onChange', () => {
    const onChange = vi.fn();
    render(<U.Tabs items={[{ id: 'a', label: 'A' }, { id: 'b', label: 'B' }]} active="a" onChange={onChange} />);
...
describe('ToggleGroup', () => {
  it('toggles values', () => {
    const onChange = vi.fn();
    render(<U.ToggleGroup options={[{ value: 'a', label: 'A' }, { value: 'b', label: 'B' }, { value: 'c', label: 'C', disabled: true }]} value={['a']} onChange={onChange} ariaLabel="t" />);
...
describe('MACInput', () => {
  it('typing + nav + backspace', () => {
    const onChange = vi.fn();
    const { container } = render(<U.MACInput label="MAC" value="ff:ff:ff:ff:ff:ff" onChange={onChange} />);
    const inputs = Array.from(container.querySelectorAll('input')) as HTMLInputElement[];
...
describe('DatePicker', () => {
  it('opens, picks a day, navigates months', () => {
    const onChange = vi.fn();
    const { container } = render(<U.DatePicker value={new Date('2025-01-15')} onChange={onChange} />);
...
describe('DateRangePicker', () => {
  it('selects a range', () => {
    const onChange = vi.fn();
    const { container } = render(<U.DateRangePicker value={{ start: new Date('2025-01-01'), end: new Date('2025-01-10') }} onChange={onChange} />);
...
describe('SortableList', () => {
  it('arrow keys reorder', () => {
    const onChange = vi.fn();
    render(<U.SortableList items={[{ id: '1', label: 'A' }, { id: '2', label: 'B' }, { id: '3', label: 'C' }]} onChange={onChange} />);
...
describe('TreeView', () => {
  it('expand/collapse + select with keyboard', () => {
    const onSelect = vi.fn();
    render(<U.TreeView nodes={[{ id: 'a', label: 'A', children: [{ id: 'b', label: 'B' }] }, { id: 'c', label: 'C' }]} onSelect={onSelect} defaultExpanded={['a']} label="T" />);
...
describe('ColumnToggle', () => {
  it('opens then toggles a column', () => {
    const onChange = vi.fn();
    render(<U.ColumnToggle columns={[{ key: 'a', label: 'A', required: true }, { key: 'b', label: 'B' }]} visible={new Set(['a', 'b'])} onChange={onChange} />);
...
describe('SplitButton', () => {
  it('primary click + opens menu + picks', () => {
    ...
    const onAction = vi.fn();
    render(<U.SplitButton label="Save" items={[{ id: 'a', label: 'Save as' }, { id: 'b', label: 'Disabled', disabled: true }]} onPrimaryClick={onPrimary} onAction={onAction} />);
...
describe('Spoiler', () => {
  it('expands and collapses', () => {
    ...
    Object.defineProperty(HTMLElement.prototype, 'scrollHeight', { configurable: true, get: () => 9999 });
    render(<U.Spoiler maxHeight={20}><div style={{ height: 200 }}>tall</div></U.Spoiler>);
    fireEvent.click(screen.getByRole('button'));
    fireEvent.click(screen.getByRole('button'));
    if (orig) Object.defineProperty(HTMLElement.prototype, 'scrollHeight', orig);
  });
  it('does not render toggle when content fits', () => {
    render(<U.Spoiler maxHeight={9999}><div>x</div></U.Spoiler>);
  });
});
...
describe('TransferList', () => {
  it('moves items source -> target -> source', () => {
    const onChange = vi.fn();
    const { container } = render(<U.TransferList defaultSource={[{ id: '1', label: 'A' }, { id: '2', label: 'B' }]} defaultTarget={[{ id: '3', label: 'C' }]} onChange={onChange} />);
  ...
  it('controlled', () => {
    render(<U.TransferList source={[{ id: '1', label: 'A' }]} target={[{ id: '2', label: 'B' }]} onChange={() => {}} />);
  });
});
...
describe('FileUpload', () => {
  it('handles drop + change', () => {
    ...
    const dropzone = container.querySelector('.file-upload, label, [class*=upload]') as HTMLElement | null;
...
describe('FilterBuilder', () => {
  it('add, edit, remove rule + change conjunction', () => {
    const onChange = vi.fn();
    render(<U.FilterBuilder fields={[{ key: 'name', label: 'Name' }, { key: 'count', label: 'Count', type: 'number' }, { key: 'kind', label: 'Kind', type: 'select', options: ['x', 'y'] }]} value={[{ id: '1', field: 'name', op: 'is', value: 'foo' }]} onChange={onChange} conjunction="and" />);
    const buttons = screen.getAllByRole('button');
    const addBtn = buttons.find((b) => /add/i.test(b.textContent ?? ''))!;
    fireEvent.click(addBtn);
    const selects = screen.getAllByRole('combobox') as HTMLSelectElement[];
    for (const s of selects) fireEvent.change(s, { target: { value: s.options[s.options.length - 1]!.value } });
    const inputs = screen.getAllByRole('textbox') as HTMLInputElement[];
    for (const i of inputs) fireEvent.change(i, { target: { value: 'bar' } });
    const removeBtn = screen.getAllByRole('button').find((b) => /remove/i.test(b.getAttribute('aria-label') ?? ''));
...
describe('Popover', () => {
  it('opens, closes on outside, on Escape', () => {
    render(<U.Popover label="open" title="T">body</U.Popover>);
...
describe('HoverCard', () => {
  ...
  it('shows after delay then hides', () => {
    render(<U.HoverCard delay={100} content={<div>tip</div>}>trigger</U.HoverCard>);
...
describe('Carousel', () => {
  it('next + prev + indicator', () => {
    render(<U.Carousel label="x" slides={[{ id: 'a', title: 'A', body: 'a' }, { id: 'b', title: 'B', body: 'b' }, { id: 'c', title: 'C', body: 'c' }]} />);
...
describe('NotificationPanel', () => {
  it('mark all read + mark single read', () => {
    ...
    render(<U.NotificationPanel open onClose={() => {}} onMarkAllRead={onMarkAll} onMarkRead={onMarkRead} notifications={[{ id: '1', title: 'a', body: 'b', time: 'now', read: false, severity: 'info' }, { id: '2', title: 'b', body: 'b', time: 'now', read: true, severity: 'warn' }]} />);
    const buttons = screen.getAllByRole('button');
    for (const b of buttons) fireEvent.click(b);
  });
});
...
describe('Toast', () => {
  ...
  it('shows and dismisses', () => {
    render(<U.Toaster />);
    act(() => { U.toast.show('a'); U.toast.success('b'); U.toast.warn('c'); U.toast.danger('d'); U.toast.info('e'); });
...
describe('ResizablePanel', () => {
  it('drags pointer to resize', () => {
    const { container } = render(<U.ResizablePanel orientation="horizontal" defaultSize={50}>{[<div key="a">A</div>, <div key="b">B</div>]}</U.ResizablePanel>);
...
describe('Topbar / Sidebar interactions', () => {
  it('Topbar clicks app + bell', () => {
    ...
    render(<U.Topbar siteName="HQ" activeApp="network" onAppChange={onAppChange} notificationCount={3} />);
    const buttons = screen.getAllByRole('button');
    for (const b of buttons) fireEvent.click(b);
  });
  it('Sidebar onChange', () => {
    const onChange = vi.fn();
    render(<U.Sidebar activeId="dash" onChange={onChange} sections={[{ title: 'Manage', items: [{ id: 'dash', label: 'Dashboard', icon: null }, { id: 'devs', label: 'Devices', icon: null, count: 2, pill: 1 }] }]} />);
...
describe('InputGroup variants', () => {
  it('renders without prefix or suffix', () => {
    render(<U.InputGroup><input /></U.InputGroup>);
  });
});

describe('Stat deltaDir + Sparkline active toggle', () => {
  it('Stat down/up/neutral', () => {
    render(<><U.Stat label="L" value="42" delta="+1" deltaDir="up" /><U.Stat label="L" value="42" delta="-1" deltaDir="down" /><U.Stat label="L" value="42" /></>);
  });
});

describe('VirtualList scroll', () => {
  it('scrolls the inner viewport', () => {
    const { container } = render(<U.VirtualList items={Array.from({ length: 200 }, (_, i) => i)} itemHeight={20} height={100} renderItem={(n) => <span>{n}</span>} />);
...
describe('Banner / Alert dismiss', () => {
  it('Banner dismiss + action click', () => {
    ...
    const action = vi.fn();
    render(<U.Banner onDismiss={onDismiss} action={{ label: 'Do', onClick: action }}>x</U.Banner>);
...
describe('Callout variants', () => {
  it('all variants', () => {
    render(<>
      <U.Callout variant="info" title="i">x</U.Callout>
      <U.Callout variant="success" title="s">x</U.Callout>
      <U.Callout variant="warn" title="w">x</U.Callout>
      <U.Callout variant="danger" title="d">x</U.Callout>
...
describe('Breadcrumb navigation', () => {
  it('clicking button without href triggers onNavigate', () => {
    ...
    render(<U.Breadcrumb items={[{ label: 'Root' }, { label: 'Sub' }, { label: 'Now' }]} onNavigate={onNavigate} />);
    const buttons = screen.getAllByRole('button');
    for (const b of buttons) fireEvent.click(b);
    expect(onNavigate).toHaveBeenCalled();
  });
  it('href items render as anchors', () => {
    render(<U.Breadcrumb items={[{ label: 'Home', href: '/' }, { label: 'Sub', href: '/sub' }, { label: 'Now' }]} />);
...
describe('Input + Textarea', () => {
  it('renders custom className/rows', () => {
    render(<><U.Input className="x" /><U.Textarea rows={6} className="x" /></>);
  });
});
...
describe('SwitchPortGrid click', () => {
  it('clicking port fires callback', () => {
    ...
    render(<U.SwitchPortGrid onPortClick={onPortClick} ports={[{ index: 1, status: 'up' }, { index: 2, status: 'down' }, { index: 3, status: 'disabled' }, { index: 4, status: 'poe' }]} />);
...
describe('JsonViewer expand', () => {
  it('toggles open state', () => {
    render(<U.JsonViewer data={{ a: { b: { c: { d: { e: 1 } } } } }} maxDepth={1} defaultExpanded />);
    const buttons = screen.getAllByRole('button');
...
describe('LogViewer auto-follow toggle', () => {
  it('scroll disables follow + clicking follow re-enables', () => {
    const { container } = render(<U.LogViewer entries={Array.from({ length: 30 }, (_, i) => ({ time: `${i}`, level: ['info', 'warn', 'error', 'debug'][i % 4] as any, message: `m${i}` }))} />);
...
describe('SelectionToolbar clear', () => {
  it('clear button fires callback + action click', () => {
    const onClear = vi.fn();
    const onAction = vi.fn();
    render(<U.SelectionToolbar count={3} actions={[{ id: 'a', label: 'Act', onClick: onAction }, { id: 'd', label: 'Delete', danger: true, onClick: onAction }]} onClear={onClear} />);
    const buttons = screen.getAllByRole('button');
    for (const b of buttons) fireEvent.click(b);
  });
});
...
describe('ExpandableRow expand', () => {
  it('toggles row', () => {
    render(<table><U.ExpandableRow row={<td>row</td>} detail={<div>detail</div>} colSpan={1} /></table>);
    fireEvent.click(screen.getByRole('button'));
    fireEvent.click(screen.getByRole('button'));
  });
});
...
describe('GroupedList collapsible', () => {
  it('toggles each group', () => {
    render(<U.GroupedList groups={[{ title: 'A', items: [{ label: 'one' }] }, { title: 'B', items: [{ label: 'two' }] }]} />);
    const buttons = screen.getAllByRole('button');
    for (const b of buttons) fireEvent.click(b);
  });
});
...
describe('Various uncontrolled defaults', () => {
  it('NumberInput uncontrolled empty', () => { render(<U.NumberInput label="N" defaultValue={5} />); });
  it('OTPInput defaultValue', () => { render(<U.OTPInput label="O" defaultValue="123" length={4} />); });
  it('IPInput defaultValue', () => { render(<U.IPInput label="IP" defaultValue="192.168.1.1" />); });
  it('MACInput defaultValue', () => { render(<U.MACInput label="M" defaultValue="aa:bb:cc:dd:ee:ff" />); });
  it('CIDRInput defaultValue', () => { render(<U.CIDRInput label="C" defaultValue="10.0.0.0/8" />); });
  it('TimePicker defaultValue', () => { render(<U.TimePicker label="T" defaultValue="13:45" />); });
  it('DurationInput defaultValue', () => { render(<U.DurationInput label="D" defaultValue={3700} />); });
  it('TagInput defaultValue', () => { render(<U.TagInput label="L" defaultValue={['a', 'b']} />); });
  it('PasswordInput defaultValue', () => { render(<U.PasswordInput label="P" defaultValue="abc" />); });
  it('Slider defaultValue', () => { render(<U.Slider label="L" defaultValue={50} />); });
  it('RangeSlider defaults', () => { render(<U.RangeSlider label="R" defaultLow={20} defaultHigh={70} />); });
  it('StarRating defaultValue', () => { render(<U.StarRating label="r" defaultValue={3} />); });
});
