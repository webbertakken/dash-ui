...
export function RidgelinePlot({ series, xRange, height = 180, ariaLabel = 'Ridgeline plot' }: RidgelinePlotProps) {
  const VW = 380;
  const VH = height;
  const PAD_L = 56;
  const PAD_R = 12;
  ...
  const evalPts = Array.from({ length: NUM_PTS }, (_, i) =>
  ...
  return (
    <div role="img" aria-label={ariaLabel} style={{ width: '100%' }}>
      ...
      >
        {xTicks.map(({ v, x }) => (
          <g key={v}>
            <line x1={x} y1={PAD_T} x2={x} y2={VH - PAD_B} stroke="rgba(255,255,255,0.06)" strokeWidth={1} />
            <text x={x} y={VH - PAD_B + 12} fill="#6E7079" fontSize={8} textAnchor="middle" fontFamily="inherit">
              {v.toFixed(0)}
            </text>
          </g>
        ))}
        {reversedRidges.map(r => (
          <g key={r.label}>
            <line x1={PAD_L} y1={r.baselineY} x2={VW - PAD_R} y2={r.baselineY} stroke="rgba(255,255,255,0.08)" strokeWidth={0.5} />
            <path d={r.d} fill={r.color} opacity={0.45} />
            <path d={r.d} fill="none" stroke={r.color} strokeWidth={1.2} opacity={0.9} />
          </g>
        ))}
        {ridges.map((r, si) => (
          <text key={si} x={PAD_L - 4} y={r.baselineY - 2} fill="#A4A7B5" fontSize={8} textAnchor="end" fontFamily="inherit">
            {r.label}
          </text>
        ))}
      </svg>
