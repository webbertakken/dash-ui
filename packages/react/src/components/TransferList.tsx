...
export function TransferList({
  ...
  function renderList(
    ...
    return (
      <div className="tl__panel">
        ...
        >
          {items.length === 0 ? (
            <li className="tl__empty" aria-disabled="true">Empty</li>
          ) : (
            items.map((item, i) => (
              <li
                key={item.id}
                ...
                onClick={() => { setFoc(i); toggle(sel, setSel, item.id); }}
              >
  ...
  return (
    <div className={`tl ${className}`.trim()}>
      {renderList(
        `${gid}-src`, sourceLabel, srcItems, selSrc, setSelSrc, focSrc, setFocSrc, srcRefs,
        () => moveRight(selSrc),
      )}
      <div className="tl__controls" role="group" aria-label="Transfer controls">
        <button type="button" className="tl__btn" aria-label={`Move all to ${targetLabel}`}
          disabled={!srcItems.length} onClick={() => moveRight(new Set(srcItems.map((i) => i.id)))}>
          {'»'}
        </button>
        <button type="button" className="tl__btn" aria-label={`Move selected to ${targetLabel}`}
          disabled={!selSrc.size} onClick={() => moveRight(selSrc)}>
          {'›'}
        </button>
        <button type="button" className="tl__btn" aria-label={`Move selected back to ${sourceLabel}`}
          disabled={!selTgt.size} onClick={() => moveLeft(selTgt)}>
          {'‹'}
        </button>
        <button type="button" className="tl__btn" aria-label={`Move all back to ${sourceLabel}`}
          disabled={!tgtItems.length} onClick={() => moveLeft(new Set(tgtItems.map((i) => i.id)))}>
          {'«'}
        </button>
      </div>
      {renderList(
        `${gid}-tgt`, targetLabel, tgtItems, selTgt, setSelTgt, focTgt, setFocTgt, tgtRefs,
        () => moveLeft(selTgt),
      )}
    </div>
  );
