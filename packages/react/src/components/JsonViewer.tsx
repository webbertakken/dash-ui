...
function JvNode({ v, k, depth, maxDepth, startOpen }: {
  v: JsVal; k?: string; depth: number; maxDepth: number; startOpen: boolean;
}) {
  ...
  if (!isArr && !isObj) {
    return (
      <div role="treeitem" aria-selected={false} className="jv-row" style={{ paddingLeft: indent }}>
        {k != null && <><span className="jv-key">&quot;{k}&quot;</span><span className="jv-colon">: </span></>}
  ...
  return (
    <div role="treeitem" aria-selected={false} aria-expanded={open} className="jv-node">
      ...
      >
        ...
        {!open && <><span className="jv-count">&thinsp;{count}&thinsp;</span><span className="jv-bracket">{cb}</span></>}
      </button>
      {open && (
...
export function JsonViewer({ data, label = 'JSON viewer', defaultExpanded = true, maxDepth = 3 }: JsonViewerProps) {
  return (
