...
export function TimePicker({
  ...
  const [internal, setInternal] = useState<[number, number]>(() =>
    parseTime(valueProp ?? defaultValue),
  )
  ...
  function handleKey(field: 0 | 1, e: KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === 'ArrowUp') {
      ...
      if (field === 0) commit(next, minutes)
      else commit(hours, next)
  ...
  function handleChange(field: 0 | 1, e: ChangeEvent<HTMLInputElement>) {
    ...
    if (field === 0) commit(clamped, minutes)
    else commit(hours, clamped)
  ...
  return (
    <div className={`time-picker-wrapper${className ? ` ${className}` : ''}`}>
      {label && (
        <label htmlFor={`${inputId}-h`} className="time-picker__label">
          {label}
        </label>
      )}
      <div
