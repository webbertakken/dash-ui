    ...
    <div ref={wrapperRef} className={`multiselect-wrapper ${className}`.trim()}>
      ...
      >
        {value.map((v) => {
          ...
          return opt ? <Tag key={v} label={opt.label} onRemove={() => toggle(v)} /> : null
        })}
        <input
          ref={inputRef}
          id={inputId}
        ...
        >
          <svg
            className={`select-chevron${open ? ' combobox-chevron-open' : ''}`}
            viewBox="0 0 16 16"
            aria-hidden="true"
            focusable={false}
          >
      ...
      {open && (
        <ul
          ...
          className="select-listbox multiselect-listbox"
        >
          {filtered.length === 0 ? (
