...
export function SankeyDiagram({
  ...
  const { positioned, paths } = useMemo(() => {
    if (!nodes.length || !links.length)
      return { positioned: [] as PositionedNode[], paths: [] as { d: string; color: string }[] }

    const nodeMap = new Map(
      nodes.map((n, i) => [n.id, { ...n, color: n.color ?? PALETTE[i % PALETTE.length] }]),
    )
    ...
    const sourcePort = new Map<string, number>(
      positioned.filter((n) => n.side === 'left').map((n) => [n.id, n.y]),
    )
    const targetPort = new Map<string, number>(
      positioned.filter((n) => n.side === 'right').map((n) => [n.id, n.y]),
    )
