...
export function StackedProgress({ segments, total, ariaLabel, showLegend = true }: StackedProgressProps) {
  const sum = total ?? segments.reduce((acc, s) => acc + s.value, 0);
  const text = ariaLabel ?? segments.map((s) => `${s.label} ${Math.round((s.value / sum) * 100)}%`).join(', ');
  return (
