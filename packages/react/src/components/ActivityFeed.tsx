        ...
        >
          ...
          <div className="af-body">
            <div className="af-header">
              <span id={`af-title-${item.id}`} className="af-title">{item.title}</span>
              <span className="af-time">{item.time}</span>
            </div>
            {item.description && (
              <p id={`af-desc-${item.id}`} className="af-desc">{item.description}</p>
            )}
          </div>
        </article>
      ))}
      {items.length === 0 && (
        <div className="af-empty" role="status">No activity</div>
      )}
      <div ref={endRef} />
    </div>
  );
