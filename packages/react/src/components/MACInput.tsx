...
function normalizeMAC(mac: string): string {
  return mac.replace(/-/g, ':').replace(/:/g, '').match(/.{1,2}/g)?.join(':') ?? mac;
}
...
export function MACInput({
  ...
  function handlePaste(e: ClipboardEvent<HTMLInputElement>) {
    ...
    if (FULL_MAC_RE.test(text)) {
      ...
      const upper = next.map((p) => p.toUpperCase()) as [string, string, string, string, string, string];
  ...
  return (
    <div className={`mac-input-wrapper ${className}`.trim()}>
      {label && (
        <label htmlFor={`${inputId}-0`} className="mac-input__label">{label}</label>
      )}
      <div
      ...
      >
        {pairs.flatMap((pair, i) => {
          ...
          if (i < 5) nodes.push(
            <span key={`c${i}`} className="mac-input__colon" aria-hidden="true">:</span>,
          );
          return nodes;
        })}
      </div>
