...
export function ParetoChart({
  ...
  const { bars, linePoints, thresholdY, pctLabels, gridLines } = useMemo(() => {
    if (!items.length) return { bars: [], linePoints: '', thresholdY: null, pctLabels: [], gridLines: [] };
  ...
  return (
    <div role="img" aria-label={ariaLabel} style={{ width: '100%' }}>
      ...
      >
        ...
        {bars.map((b) => (
          <rect key={b.key} x={b.x} y={b.y} width={b.w} height={b.h} fill={b.color} fillOpacity={0.85} rx={2} />
        ))}
        {thresholdY && (
          <line
            x1={PAD.l} y1={thresholdY} x2={VW - PAD.r} y2={thresholdY}
        ...
        {pctLabels.map((pl) => (
          <text key={pl.label} x={VW - PAD.r + 4} y={pl.y} fill="#6E7079" fontSize={8} fontFamily="inherit">
            {pl.label}
          </text>
        ))}
        {bars.map((b, i) => (
