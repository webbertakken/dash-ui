...
const GW = 38
const GH = 28
const LEVELS = [0.12, 0.3, 0.54, 0.78]

function buildGrid(pts: Array<{ gx: number; gy: number }>, bw: number): number[][] {
  const bw2 = bw * bw
  const g: number[][] = Array.from({ length: GH + 1 }, () =>
    Array.from({ length: GW + 1 }, () => 0),
  )
  for (let r = 0; r <= GH; r++) {
    for (let c = 0; c <= GW; c++) {
      let d = 0
...
function marchCell(tl: number, tr: number, bl: number, br: number, t: number): Array<[P, P]> {
  const a = tl >= t ? 1 : 0
  const b = tr >= t ? 1 : 0
  const c = bl >= t ? 1 : 0
  const d = br >= t ? 1 : 0
