...
export function OTPInput({
  ...
  const digits = valueProp !== undefined
    ? Array.from({ length }, (_, i) => valueProp[i] ?? '')
    : internal;
  ...
  return (
    <div className={`otp-input-wrapper${className ? ` ${className}` : ''}`}>
      {label && (
        <label htmlFor={`${inputId}-0`} className="otp-input__label">{label}</label>
      )}
      <div
      ...
      >
        {digits.flatMap((d, i) => {
          ...
          if (i === half) {
            nodes.push(
              <span key="sep" className="otp-input__separator" aria-hidden="true">&ndash;</span>,
            );
          }
          nodes.push(
            <input
