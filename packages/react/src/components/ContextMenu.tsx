...
export function ContextMenu({ items, x, y, open, onClose, onAction, label = 'Context menu' }: ContextMenuProps) {
  const uid = useId();
  ...
  function onKeyDown(e: KeyboardEvent) {
    ...
    else if (e.key === 'ArrowDown') { e.preventDefault(); setActiveIdx((i) => Math.min(i + 1, actionItems.length - 1)); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); setActiveIdx((i) => Math.max(i - 1, 0)); }
    else if (e.key === 'Home') { e.preventDefault(); setActiveIdx(0); }
    else if (e.key === 'End') { e.preventDefault(); setActiveIdx(actionItems.length - 1); }
