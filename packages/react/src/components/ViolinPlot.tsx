...
export function ViolinPlot({ series, yRange, height = 160, ariaLabel = 'Violin plot' }: ViolinPlotProps) {
  const VW = 380;
  const VH = height;
  const PAD_L = 28;
  const PAD_R = 8;
  ...
  return (
    <div role="img" aria-label={ariaLabel} style={{ width: '100%' }}>
      ...
      >
        {ticks.map(({ v, y }) => (
          <g key={v}>
            <line x1={PAD_L} y1={y} x2={VW - PAD_R} y2={y} stroke="rgba(255,255,255,0.06)" strokeWidth={1} />
            <text x={PAD_L - 3} y={y + 3} fill="#6E7079" fontSize={8} textAnchor="end" fontFamily="inherit">
              {v.toFixed(0)}
            </text>
          </g>
        ))}
        {violins.map((v, si) => (
          <g key={si}>
            ...
            <line
              ...
              strokeLinecap="round"
            />
            <text x={v.cx} y={VH - 5} fill="#6E7079" fontSize={8} textAnchor="middle" fontFamily="inherit">
              {v.label}
            </text>
          </g>
        ))}
