...
function pack(radii: number[]): { cx: number; cy: number }[] {
  ...
  for (let i = 0; i < radii.length; i++) {
    const r = radii[i]
    if (i === 0) {
      pos.push({ cx: 0, cy: 0, r })
      continue
    }
    if (i === 1) {
      pos.push({ cx: pos[0].r + r, cy: 0, r })
      continue
    }
...
export function CirclePacking({
  ...
  const layout = useMemo(() => {
    ...
    const radii = sorted.map((it) =>
      Math.max(MIN_R, Math.round(MAX_R * Math.sqrt(it.value / maxV))),
    )
