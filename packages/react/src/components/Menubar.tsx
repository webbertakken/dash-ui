...
export function Menubar({ menus, onAction, label = 'Menu', className }: MenubarProps) {
  ...
  function handleTriggerKeyDown(e: KeyboardEvent, idx: number) {
    ...
    if (openIdx === null) {
      if (e.key === 'ArrowRight') { e.preventDefault(); triggerRefs.current[(idx + 1) % count]?.focus(); }
      else if (e.key === 'ArrowLeft') { e.preventDefault(); triggerRefs.current[(idx - 1 + count) % count]?.focus(); }
      ...
      else if (e.key === 'End') { e.preventDefault(); triggerRefs.current[count - 1]?.focus(); }
    } else {
      const menu = menus[openIdx];
      const eligible = menu.items.filter((i) => !i.separator && !i.disabled);
      if (e.key === 'Escape') { e.preventDefault(); closeMenu(); triggerRefs.current[openIdx]?.focus(); }
      else if (e.key === 'ArrowDown') { e.preventDefault(); setActiveItemIdx((i) => Math.min(i + 1, eligible.length - 1)); }
      else if (e.key === 'ArrowUp') { e.preventDefault(); setActiveItemIdx((i) => Math.max(i - 1, 0)); }
      else if (e.key === 'Home') { e.preventDefault(); setActiveItemIdx(0); }
      else if (e.key === 'End') { e.preventDefault(); setActiveItemIdx(eligible.length - 1); }
          ...
          <div key={menu.id} className="menubar-menu">
            ...
            {isOpen && (
              <ul id={menuId} role="menu" aria-label={menu.label} className="menubar-dropdown">
                {menu.items.map((item) => {
                  if (item.separator) {
                    return <li key={item.id} role="separator" className="menubar-sep" aria-hidden="true" />;
                  }
                  ...
                  return (
                    <li
                      ...
                      data-active={eligibleIdx === activeItemIdx && !item.disabled ? 'true' : undefined}
                      className="menubar-item"
                      onMouseEnter={() => { if (!item.disabled) setActiveItemIdx(eligibleIdx); }}
                      onMouseDown={(e) => {
                        e.preventDefault();
