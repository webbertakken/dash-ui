...
export function HorizonChart({ series, xLabels, bands = 3, ariaLabel = 'Horizon chart' }: HorizonChartProps) {
    ...
    <div role="img" aria-label={ariaLabel} style={{ width: '100%' }}>
      ...
      >
        {series.map((s, si) => {
          ...
          return (
            <g key={si}>
              <text x={LABEL_W - 6} y={rowY + ROW_H / 2 + 4} textAnchor="end" fill="#C8C9D0" fontSize={9} fontFamily="inherit">
                {s.label}
              </text>
              <rect x={LABEL_W} y={rowY} width={chartW} height={ROW_H} fill="rgba(255,255,255,0.04)" rx={2} />
