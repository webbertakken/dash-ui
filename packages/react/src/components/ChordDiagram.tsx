...
export function ChordDiagram({ nodes, matrix, ariaLabel = 'Chord diagram' }: ChordDiagramProps) {
  const { arcs, chords, labels } = useMemo(() => {
    ...
    if (!grand)
      return {
        arcs: [] as { d: string; fill: string }[],
        chords: [] as { d: string; fill: string }[],
    ...
    for (let i = 0; i < n; i++) {
      for (let j = i + 1; j < n; j++) {
        const vij = matrix[i][j],
          vji = matrix[j][i]
        if (!vij && !vji) continue
        const a1 = nodeA[i].offsets[j],
          a2 = a1 + vij * scale
        const b1 = nodeA[j].offsets[i],
          b2 = b1 + vji * scale
        chords.push({
          d: ribbon(a1, a2, b1, b2),
          fill: (nodes[i].color ?? PALETTE[i % PALETTE.length]) + '55',
        })
      }
    }
    ...
    const labels = nodeA.map((a, i) => {
      ...
      return {
        x: CX + R_LBL * Math.cos(mid),
        y: CY + R_LBL * Math.sin(mid) + 4,
        anchor,
        text: nodes[i].label,
      }
    })
