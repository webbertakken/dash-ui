...
export function AnnotatedTimeSeries({
  ...
  const { linePath, areaPath, gridLines, xLabels, markers } = useMemo(() => {
    if (data.length < 2) return { linePath: '', areaPath: '', gridLines: [], xLabels: [], markers: [] };
    const n = data.length;
  ...
  return (
    <div role="img" aria-label={ariaLabel} style={{ width: '100%' }}>
      ...
      >
        {gridLines.map((g, i) => (
          <g key={i}>
            <line x1={PAD.l} y1={g.y} x2={VW - PAD.r} y2={g.y} stroke="rgba(255,255,255,0.06)" strokeWidth={1} />
            <text x={PAD.l - 4} y={g.y + 3} fill="#6E7079" fontSize={8} textAnchor="end" fontFamily="inherit">{g.label}</text>
          </g>
        ))}
        ...
        {markers.map((m, i) => (
          <g key={i}>
            <line x1={m.x} y1={PAD.t} x2={m.x} y2={PAD.t + m.chartH} stroke={m.color} strokeWidth={1} strokeDasharray="3,2" opacity={0.85} />
            <text x={m.x + 2} y={PAD.t + 8} fill={m.color} fontSize={7} fontFamily="inherit">{m.label}</text>
          </g>
        ))}
