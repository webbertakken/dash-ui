...
export function SunburstChart({ root, ariaLabel = 'Sunburst chart' }: SunburstChartProps) {
  const { paths, labels, total } = useMemo(() => {
    ...
    if (!total) return { paths: [] as { d: string; fill: string }[], labels: [] as { x: number; y: number; anchor: 'start' | 'end' | 'middle'; text: string }[], total: 0 };
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (span > 0.22) {
        ...
        const anchor: 'start' | 'end' | 'middle' = Math.cos(midA) > 0.15 ? 'start' : Math.cos(midA) < -0.15 ? 'end' : 'middle';
  ...
  return (
    <div role="img" aria-label={ariaLabel} style={{ width: '100%' }}>
      <svg viewBox="0 0 320 320" style={{ width: '100%', height: 'auto', display: 'block' }} aria-hidden="true" focusable="false">
        ...
        <text x={CX} y={CY + 12} textAnchor="middle" fill="#6E7079" fontSize={10} fontFamily="inherit">
          {root.label}
        </text>
      </svg>
    </div>
