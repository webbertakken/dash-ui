...
export function DurationInput({
  ...
  const refs = useRef<[HTMLInputElement | null, HTMLInputElement | null, HTMLInputElement | null]>([
    null,
    null,
    null,
  ])
  ...
  return (
    <div className={`dur-input-wrapper${className ? ` ${className}` : ''}`}>
      {label && (
        <label htmlFor={`${inputId}-h`} className="dur-input__label">
          {label}
        </label>
      )}
      <div
      ...
      >
        {FIELDS.flatMap(({ idx, val, ariaLabel, ariaMax }, i) => {
          ...
          if (i < 2)
            nodes.push(
              <span key={`c${idx}`} className="dur-input__sep" aria-hidden="true">
                :
              </span>,
            )
          return nodes
