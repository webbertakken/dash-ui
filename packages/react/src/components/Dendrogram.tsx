...
function placeNodes(
  ...
  if (isLeaf) {
    ...
    counter.v++;
    return { id: n.id, label: n.label, x: depth * colWidth, y, color: nodeColor, isLeaf: true, children: [] };
  }
  ...
  return { id: n.id, label: n.label, x: depth * colWidth, y, color: nodeColor, isLeaf: false, children };
}
...
export function Dendrogram({ root, colWidth = 130, rowHeight = 28, ariaLabel = 'Dendrogram' }: DendrogramProps) {
  const PAD_L = 12;
  const PAD_V = 10;
        ...
        {nodes.map((n) => (
          <g key={n.id}>
            ...
            {n.isLeaf ? (
              ...
              <text x={n.x} y={n.y - 7} fill="#8B909E" fontSize={9} fontFamily="inherit" textAnchor="middle">
                {n.label}
              </text>
            )}
          </g>
