...
export function WaterfallChart({ bars, height = 160, ariaLabel = 'Waterfall chart' }: WaterfallChartProps) {
  const { rects, connectors, labels } = useMemo(() => {
    ...
    const rects = bars.map((b, i) => {
      ...
      return { x: x.toFixed(1), y: y0.toFixed(1), w: barW.toFixed(1), h: Math.max(y1 - y0, 1).toFixed(1), color, topV: top, baseV: base };
    });
  ...
  return (
    <div role="img" aria-label={ariaLabel} style={{ width: '100%' }}>
      ...
      >
        ...
        {connectors.map((c, i) => (
          <line key={i} x1={c.x1} y1={c.y} x2={c.x2} y2={c.y} stroke="rgba(255,255,255,0.2)" strokeWidth={1} strokeDasharray="2 2" />
        ))}
        {rects.map((r, i) => (
          <rect key={i} x={r.x} y={r.y} width={r.w} height={r.h} fill={r.color} fillOpacity={0.85} rx={2} />
        ))}
        {labels.map((l, i) => (
          <text key={i} x={l.x} y={l.y} textAnchor="middle" fill="#6E7079" fontSize={9} fontFamily="inherit">
            {l.label}
          </text>
        ))}
      </svg>
