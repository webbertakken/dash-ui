...
export function CalendarHeatmap({ data, maxValue, ariaLabel = 'Calendar heatmap' }: CalendarHeatmapProps) {
  ...
  return (
    <div role="img" aria-label={ariaLabel} style={{ width: '100%', overflowX: 'auto' }}>
      ...
      >
        ...
        {monthLabels.map((m) => (
          <text key={`${m.label}-${m.x}`} x={m.x} y={TOP - 3} fill="#6E7079" fontSize={7} fontFamily="inherit">
            {m.label}
          </text>
        ))}
        {cells.map((c, i) => (
