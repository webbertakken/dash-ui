...
<section
  class="relative overflow-hidden rounded-lg border border-border-1 bg-divider {klass}"
...
>
    ...
    {#each slides as slide, i (slide.id)}
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          <div class="text-15 font-semibold text-text-1">{slide.title}</div>
  ...
  <div class="flex items-center justify-between border-t border-border-1 bg-bg-page px-2 py-1.5">
    <button
      type="button"
    ...
    <div role="tablist" aria-label="Choose slide to display" class="flex items-center gap-1.5">
      {#each slides as slide, i (slide.id)}
        <button
          ...
          data-active={i === index ? 'true' : undefined}
          class="h-1.5 w-1.5 cursor-pointer rounded-full border-0 bg-border-3 transition-all duration-150 hover:bg-text-3 data-[active=true]:w-4 data-[active=true]:bg-brand-05"
          onclick={() => (index = i)}
        ></button>
      {/each}
    </div>
