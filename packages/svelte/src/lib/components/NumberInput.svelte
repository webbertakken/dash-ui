...
<div class="inline-flex flex-col gap-1 {className}">
  ...
  {/if}
  <div class="inline-flex h-[34px] items-stretch overflow-hidden rounded-md border border-border-2 bg-bg-page transition-colors duration-100 focus-within:border-brand-05">
    <button
      type="button"
    ...
    <input
      ...
      {value}
      {disabled}
      class="w-[6ch] min-w-[40px] border-0 bg-transparent px-1 text-center text-13 text-text-1 outline-none [appearance:textfield] [-moz-appearance:textfield] [&::-webkit-outer-spin-button]:m-0 [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:m-0 [&::-webkit-inner-spin-button]:appearance-none"
      onchange={handleChange}
      onkeydown={handleKey}
    />
    {#if suffix}
      <span aria-hidden="true" class="flex items-center whitespace-nowrap pl-0.5 pr-2 text-12 text-text-4">{suffix}</span>
    {/if}
    <button
      type="button"
