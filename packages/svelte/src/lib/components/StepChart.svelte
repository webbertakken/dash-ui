...
<script lang="ts">
  ...
  export let series: StepChartSeries[] = [];
  export let labels: string[] = [];
  export let height: number = 160;
  export let ariaLabel: string = 'Step chart';
  ...
  function stepPath(s: StepChartSeries): string {
    if (!s.values.length) return '';
    let d = `M ${toX(0).toFixed(1)},${toY(s.values[0]).toFixed(1)}`;
    for (let i = 1; i < s.values.length; i++) {
      d += ` H ${toX(i).toFixed(1)} V ${toY(s.values[i]).toFixed(1)}`;
    }
    return d;
  }

  function stepArea(s: StepChartSeries): string {
    if (!s.values.length) return '';
    const bot = (PAD.t + chartH).toFixed(1);
    let d = `M ${toX(0).toFixed(1)},${bot} V ${toY(s.values[0]).toFixed(1)}`;
    for (let i = 1; i < s.values.length; i++) {
      d += ` H ${toX(i).toFixed(1)} V ${toY(s.values[i]).toFixed(1)}`;
    }
    d += ` H ${toX(s.values.length - 1).toFixed(1)} V ${bot} Z`;
    return d;
  }

  $: gridLines = [0, 0.25, 0.5, 0.75, 1].map((f) => PAD.t + (1 - f) * chartH);
  $: paths = series.map((s) => ({ color: s.color, label: s.label, d: stepPath(s) }));
  $: areas = series.map((s) => ({ color: s.color, label: s.label, d: stepArea(s) }));
</script>
