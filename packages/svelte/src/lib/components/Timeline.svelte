...
<ol class="relative m-0 list-none p-0 {className}" aria-label="Event timeline">
  {#each events as e (e.id)}
    <li class="relative flex gap-3 pb-3 pl-4 last:pb-0 [&:not(:last-child)]:before:absolute [&:not(:last-child)]:before:bottom-0 [&:not(:last-child)]:before:left-[5px] [&:not(:last-child)]:before:top-4 [&:not(:last-child)]:before:w-px [&:not(:last-child)]:before:bg-row-active [&:not(:last-child)]:before:content-['']">
      <span
      ...
      <div class="min-w-0 flex-1">
        <div class="flex items-baseline justify-between gap-2">
          <span class="font-medium text-text-1">{e.title}</span>
          <time class="shrink-0 text-11 text-text-4 tabular-nums">{e.time}</time>
        </div>
        {#if e.description}
