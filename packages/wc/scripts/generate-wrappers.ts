#!/usr/bin/env tsx
// Reads every Svelte 4 component in @dash-ui/svelte and emits a thin custom
...
// listed in SKIP. They still ship in the @dash-ui/svelte package, just not as
...
<script>
  import Original from '@dash-ui/svelte/components/${name}.svelte';
...
const indexLines = [
  ...
  '',
  `export const REGISTERED_TAGS = ${JSON.stringify(generated.map((g) => g.tag), null, 2)};`,
  '',
];
