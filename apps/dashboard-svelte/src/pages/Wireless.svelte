<script lang="ts">
  ...
  const heatData = (() => {
    ...
    for (let y = 0; y < 24; y++) {
      const row: number[] = [];
      for (let x = 0; x < 48; x++) {
        row.push(Math.max(0, Math.min(1, 0.4 + 0.5 * Math.sin(x * 0.4 + y * 0.2) + 0.3 * Math.sin(x * 0.15) + 0.25 * (r() - 0.5))));
      }
      rows.push(row);
    }
    return rows;
  ...
  const BAND_OPTIONS: SegmentOption[] = [
    { value: '2.4', label: '2.4 GHz' },
    { value: '5', label: '5 GHz' },
    { value: '6', label: '6 GHz' },
  ];

  const TIME_OPTIONS: SegmentOption[] = [
  ...
  const apRadar: RadarSeries[] = [
    ...
    { label: 'Open office', color: '#00C875', values: [0.50, 0.38, 1.00, 1.0, 0.72] },
  ...
  const apViolin: ViolinSeries[] = [
    { label: 'Recep.', color: '#00C875', values: [-95,-92,-94,-91,-96,-93,-90,-97,-94,-88,-92,-95,-91,-93,-87,-94,-96,-90,-93,-95] },
    { label: 'Open',   color: '#F5A623', values: [-97,-95,-99,-96,-93,-100,-98,-94,-97,-95,-92,-96,-99,-97,-94,-98,-96,-93,-100,-97] },
    { label: 'Meet.B', color: '#FF7B7B', values: [-102,-99,-97,-101,-104,-98,-97,-103,-100,-96,-101,-99,-97,-103,-100,-97,-101,-98,-97,-102] },
    { label: 'Ware.',  color: '#00C875', values: [-95,-92,-96,-91,-93,-90,-96,-93,-91,-94,-90,-95,-92,-93,-91,-94,-92,-90,-93,-95] },
    { label: 'Patio',  color: '#FF7B7B', values: [-104,-101,-99,-103,-106,-100,-99,-104,-102,-98,-103,-101,-99,-104,-101,-99,-103,-100,-99,-103] },
  ];
  ...
  const rssiSwarm: BeeswarmSeries[] = [
    { label: 'Recep.', color: '#006FFF', points: [-92,-90,-94,-88,-91,-95,-89,-93,-87,-91,-96,-88,-92,-90,-94,-93,-89,-91,-87,-95] },
    { label: 'Open',   color: '#00C875', points: [-98,-96,-100,-94,-97,-101,-95,-99,-93,-97,-102,-95,-98,-96,-100,-99,-95,-97,-93,-101] },
    { label: 'Meet.B', color: '#F5A623', points: [-104,-101,-99,-106,-103,-100,-105,-102,-98,-104,-107,-101,-104,-102,-100,-106,-103,-99,-105,-102] },
    { label: 'Ware.',  color: '#A78BFA', points: [-91,-89,-93,-87,-90,-94,-88,-92,-86,-90,-95,-88,-91,-89,-93,-92,-88,-90,-86,-94] },
  ];
...
<div class="grid">
  ...
  <Card span={12}>
    ...
    <table>
      <caption class="sr-only">Access points (5 GHz)</caption>
      <thead>
        <tr><th scope="col">AP</th><th scope="col">Channel · Width</th><th scope="col">Utilization</th><th scope="col">Noise floor</th><th scope="col">Clients</th></tr>
      </thead>
      <tbody>
        {#each filteredAPS as [name, ch, util, noise, clients] (name)}
          <tr>
  ...
  <Card span={6}>
    <h3>AP RF quality</h3>
    <div style="font-size:11px;color:#6E7079;margin-bottom:8px;">Utilization (x) vs noise floor (y) · bubble = client count</div>
    <BubbleChart
      points={apScatter}
      xRange={[0, 100]}
      yRange={[-102, -90]}
    ...
    <div style="display:flex;gap:12px;margin-top:8px;font-size:11px;color:#A4A7B5;">
      <span style="display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#00C875;display:inline-block;"></span> Good</span>
      <span style="display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#F5A623;display:inline-block;"></span> Fair</span>
      <span style="display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#FF7B7B;display:inline-block;"></span> Poor</span>
    </div>
  </Card>

  <Card span={6}>
    <h3>AP performance comparison</h3>
    <div style="font-size:11px;color:#6E7079;margin-bottom:4px;">Signal · Capacity · Clients · Width · Score</div>
    <RadarChart
      series={apRadar}
  ...
  <Card span={6}>
    ...
    <div style="font-size:11px;color:#6E7079;margin-bottom:8px;">RSSI range per AP (dBm) · box = IQR · line = median</div>
    <BoxPlot
      series={apBoxPlot}
      yRange={[-112, -82]}
      height={150}
    ...
    />
    <div style="display:flex;gap:12px;margin-top:8px;font-size:11px;color:#A4A7B5;">
      <span style="display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#00C875;display:inline-block;"></span> &gt;−95 dBm</span>
      <span style="display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#F5A623;display:inline-block;"></span> −95–−98</span>
      <span style="display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#FF7B7B;display:inline-block;"></span> &lt;−98 dBm</span>
    </div>
  </Card>

  <Card span={6}>
    <h3>Client count per AP</h3>
    <div style="font-size:11px;color:#6E7079;margin-bottom:8px;">Current week vs previous week</div>
    <DotPlot
      items={apDotPlot}
      min={0}
      max={50}
  ...
  <Card span={6}>
    ...
    <div style="font-size:11px;color:#6E7079;margin-bottom:8px;">Sample count per signal-strength bin (dBm) · all APs</div>
    <Histogram
      bins={rssiHistogram}
      height={160}
      color="#006FFF"
  ...
  <Card span={6}>
    ...
    <div style="font-size:11px;color:#6E7079;margin-bottom:8px;">Distribution shape (dBm) · line = median</div>
    <ViolinPlot
      series={apViolin}
      yRange={[-110, -84]}
      height={160}
  ...
  <Card span={6}>
    ...
    <div style="font-size:11px;color:#6E7079;margin-bottom:8px;">Parallel coordinates: utilisation · SNR · clients · width · uptime</div>
    <ParallelCoordinates
      axes={apParallelAxes}
    ...
    />
    <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:8px;font-size:11px;color:#A4A7B5;">
      {#each apParallelSeries as s (s.label)}
        <span style="display:flex;align-items:center;gap:4px;">
  ...
  <Card span={6}>
    ...
    <div style="font-size:11px;color:#6E7079;margin-bottom:8px;">Signal-to-noise ratio per 3 h interval (dB) · open/close = period start/end</div>
    <CandlestickChart
      bars={snrCandles}
      yRange={[10, 33]}
      height={160}
    ...
    />
    <div style="display:flex;gap:12px;margin-top:8px;font-size:11px;color:#A4A7B5;">
      <span style="display:flex;align-items:center;gap:4px;"><span style="width:10px;height:10px;background:#00C875;display:inline-block;"></span> Improved</span>
      <span style="display:flex;align-items:center;gap:4px;"><span style="width:10px;height:10px;background:#FF7B7B;display:inline-block;"></span> Degraded</span>
    </div>
  </Card>

  <Card span={6}>
    <h3>Client RSSI distribution per AP</h3>
    <div style="font-size:11px;color:#6E7079;margin-bottom:8px;">Each dot = one client sample · dBm · last hour</div>
    <BeeswarmChart
      series={rssiSwarm}
      yRange={[-110, -80]}
      height={180}
  ...
  <Card span={6}>
    ...
    <div style="font-size:11px;color:#6E7079;margin-bottom:8px;">2D hexbin · x = RSSI (dBm) · y = SNR (dB) · brightness = sample density</div>
    <HexbinChart
      points={hexbinPts}
      xRange={[-105, -50]}
      yRange={[0, 42]}
  ...
  <Card span={6}>
    ...
    <div style="font-size:11px;color:#6E7079;margin-bottom:8px;">Iso-density contours · x = RSSI (dBm) · y = SNR (dB) · dots = samples</div>
    <ContourPlot
      points={contourPts}
      xRange={[-105, -50]}
      yRange={[0, 42]}
