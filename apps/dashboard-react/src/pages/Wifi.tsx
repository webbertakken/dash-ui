...
import { Card, Button, SearchBox, Pill, HealthBar, Toggle, InlineEdit, SortableList, PlusIcon, WifiIcon, AreaChart, MirroredBarChart, RidgelinePlot, HorizonChart } from '@dash-ui/react';
import type { MirroredBarItem, RidgelineSeries, HorizonSeries, SortableItem } from '@dash-ui/react';
  ...
  return (
    <>
      ...
      <div className="grid" style={{ paddingTop: 0 }}>
        <Card span={12}>
          <h3>WiFi Traffic by Band <span className="unit">Last 24 h · Mbps</span></h3>
          <AreaChart
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 8, fontSize: 12 }}>
            {WIFI_TRAFFIC_SERIES.map(({ label, color }) => (
              <span key={label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ width: 10, height: 10, background: color, borderRadius: 2, display: 'inline-block' }} aria-hidden="true" />
      ...
      <div className="grid" style={{ paddingTop: 0 }}>
        <Card span={6}>
          <h3>TX / RX per SSID <span className="unit">Mbps · last 5 min</span></h3>
          <MirroredBarChart
            items={SSID_TXRX}
            leftLabel="Download"
            rightLabel="Upload"
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 8, fontSize: 12 }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ width: 10, height: 10, background: '#006FFF', borderRadius: 2, display: 'inline-block' }} aria-hidden="true" />
              <span style={{ color: '#A4A7B5' }}>Download</span>
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ width: 10, height: 10, background: '#00C8C8', borderRadius: 2, display: 'inline-block' }} aria-hidden="true" />
        ...
        <Card span={6}>
          ...
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>RSSI density (dBm) · all connected clients</div>
          <RidgelinePlot
            series={SSID_RSSI}
            xRange={[-95, -58]}
            height={180}
        ...
        <Card span={12}>
          ...
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>Horizon chart — band intensity = relative client count</div>
          <HorizonChart
            series={SSID_CLIENTS}
            xLabels={HORIZON_X}
            bands={3}
      ...
      <div className="grid" style={{ paddingTop: 0 }}>
        <Card span={6}>
          <h3>SSID Priority <span className="unit">drag or use arrow keys to reorder</span></h3>
          <SortableList
            items={ssidOrder}
