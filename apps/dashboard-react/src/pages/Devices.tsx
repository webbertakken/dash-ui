...
import type { ActionMenuItem, KanbanColumn, ColumnDef } from '@w5-ui/react'
...
export function Devices({ onAdopt }: DevicesProps) {
  ...
  const BOARD_COLS: {
    ...
    title: string
    color: string
    match: (r: DeviceRow) => boolean
  }[] = [
    {
      id: 'connected',
      title: 'Connected',
      color: '#00B070',
      match: (r) => r[7].startsWith('Connected'),
    },
    {
      id: 'updating',
      title: 'Updating',
      color: '#F5A623',
      match: (r) => r[7].startsWith('Updating'),
    },
    {
      id: 'adopting',
      title: 'Adopting',
      color: '#006FFF',
      match: (r) => r[7].startsWith('Adopting'),
    },
  ...
  const [boardCols, setBoardCols] = useState<KanbanColumn[]>(() =>
    BOARD_COLS.map((col) => ({
      ...
      title: col.title,
      color: col.color,
      cards: DEVICES.filter(col.match).map((r) => ({
        id: r[2],
        title: r[0],
        subtitle: r[1],
        meta: r[7],
  ...
  return (
    <>
      ...
      {tab === 'board' ? (
        ...
        <div style={{ padding: '0 24px 24px' }}>
          ...
          >
            ...
            <thead>
              <tr>
                <SortHeader sortKey="name" activeKey={sortKey} dir={dir} onSort={handleSort}>
                  Name / Model
                </SortHeader>
                {visibleCols.has('mac') && <th scope="col">MAC / IP</th>}
                {visibleCols.has('site') && <th scope="col">Site</th>}
                {visibleCols.has('uptime') && (
                ...
                <tr key={r[2]}>
                      ...
                      content={
                        <>
                          ...
                          <div className="hovercard-row">
                            <span className="hc-key">Model</span>
                            <span className="hc-val">{r[1]}</span>
                          </div>
                          <div className="hovercard-row">
                            <span className="hc-key">MAC</span>
                            <span className="hc-val mac">{r[2]}</span>
                          </div>
                          <div className="hovercard-row">
                            <span className="hc-key">IP</span>
                            <span className="hc-val mac">{r[3]}</span>
                          </div>
                          <div className="hovercard-row">
                            <span className="hc-key">Uptime</span>
                            <span className="hc-val">{r[4]}</span>
                          </div>
                          <div className="hovercard-row">
                            <span className="hc-key">Status</span>
                            <span className="hc-val" style={{ color: r[8] }}>
                              {r[7]}
                            </span>
                          </div>
                        </>
                  ...
                  <td style={{ textAlign: 'right', width: 32 }}>
                    <ActionMenu
                      ...
                      onAction={(id) => {
                        if (id === 'details') setDrawerDevice(r)
                        if (id === 'forget') setForgetDevice(r)
                      }}
                      label={`Actions for ${r[0]}`}
                    />
                  </td>
                </tr>
              ))}
      ...
      <ConfirmDialog
        ...
        description={
          forgetDevice
            ? `${forgetDevice[0]} will be removed from this site and will need to be re-adopted.`
            : undefined
        }
        confirmLabel="Forget"
        variant="danger"
          ...
          <>
            <KVTable
              ...
              rows={[
                ...
                {
                  label: 'Status',
                  value: <span style={{ color: drawerDevice[8] }}>{drawerDevice[7]}</span>,
                },
              ]}
            />
            <div
            ...
            <JsonViewer
              ...
              data={{
                model: drawerDevice[1],
                mac: drawerDevice[2],
                ip: drawerDevice[3],
                uptime: drawerDevice[4],
                clients: drawerDevice[5],
                firmware: '9.4.21',
                features: { vlan: true, qos: false, ipv6: true },
                ports: { count: 8, poe: 4, sfp: 2 },
              }}
              maxDepth={2}
            />
          </>
