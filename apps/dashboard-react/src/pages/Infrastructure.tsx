...
import { Card, Button, Pill, DownloadIcon, ProgressBar, Gauge, Stat, BulletChart, GanttChart, SparklineMatrix, ThresholdAreaChart, QuadrantChart, CorrelationMatrix, FlameGraph, ResizablePanel, CountUp, ToggleGroup, StackedProgress } from '@dash-ui/react';
import type { GanttTask, SparklineMatrixRow, QuadrantPoint, FlameNode, ToggleGroupOption, StackedProgressSegment } from '@dash-ui/react';

const SYSTEM_METRICS = [
  { label: 'CPU', value: 41, target: 70, ranges: [50, 75] as [number, number], color: '#006FFF' },
  { label: 'Memory', value: 38, target: 60, ranges: [50, 70] as [number, number], color: '#7FB6FF' },
  { label: 'NPU', value: 72, target: 80, ranges: [60, 80] as [number, number], color: '#4797FF' },
  { label: 'Temperature', value: 45, target: 65, ranges: [60, 75] as [number, number], color: '#F5A623' },
];
...
export function Infrastructure() {
  ...
  const [visibleIfaces, setVisibleIfaces] = useState<string[]>(ALL_IFACE_THROUGHPUT.map((r) => r.label));
  ...
  return (
    <>
      ...
      <div className="grid">
        ...
        <Stat label="Clients" value={<CountUp to={142} />} unit="42 wired · 100 wireless" span={3} />
        <Stat label="PoE budget" value={<CountUp to={312} suffix=" W" />} unit="of 600 W" span={3} />
        <Stat label="Storage" value={<CountUp to={41} suffix="%" />} unit="1.64 / 4.0 TB" span={3} />
        ...
        <Card span={6}>
          <h3>Memory breakdown <span className="unit">16 GB total</span></h3>
          <div style={{ marginBottom: 12, color: '#6E7079', fontSize: 11 }}>Used 6.2 GB · Cached 2.1 GB · Free 7.7 GB</div>
          <StackedProgress
        ...
        <Card span={6}>
          <h3>Storage breakdown <span className="unit">4.0 TB total</span></h3>
          <div style={{ marginBottom: 12, color: '#6E7079', fontSize: 11 }}>System 0.82 TB · Recordings 0.54 TB · Backups 0.28 TB · Free 2.36 TB</div>
          <StackedProgress
        ...
        <Card span={12}>
          <h3>Maintenance schedule <span className="unit">This week</span></h3>
          <GanttChart
            tasks={MAINTENANCE}
        ...
        <Card span={12}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
            <h3 style={{ margin: 0 }}>Interface throughput <span className="unit">Last 24 h</span></h3>
            <ToggleGroup
        ...
        <Card span={12}>
          <h3>CPU load <span className="unit">Last 24 h · threshold 80%</span></h3>
          <ThresholdAreaChart
            values={CPU_LOAD}
            labels={CPU_LABELS}
            threshold={80}
        ...
        <Card span={12}>
          <h3>CPU process breakdown <span className="unit">Current snapshot</span></h3>
          <FlameGraph
            root={CPU_FLAME}
            height={160}
        ...
        <Card span={12}>
          ...
          >
            <div style={{ padding: '0 12px 0 0', height: '100%', overflow: 'auto' }}>
              <h3>Device load quadrant <span className="unit">CPU % vs Memory %</span></h3>
              <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>x = CPU · y = Memory · threshold 60%</div>
              <QuadrantChart
                points={DEVICE_LOAD}
                xThreshold={60}
                yThreshold={60}
            ...
            <div style={{ padding: '0 0 0 12px', height: '100%', overflow: 'auto' }}>
              <h3>Metric correlations <span className="unit">CPU · Mem · NPU · Temp</span></h3>
              <CorrelationMatrix
