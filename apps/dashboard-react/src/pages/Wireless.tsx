...
import { Card, Pill, SegmentedControl, RangeSlider, HeatMap, BubbleChart, RadarChart, BoxPlot, DotPlot, Histogram, ViolinPlot, ParallelCoordinates, CandlestickChart, BeeswarmChart, HexbinChart, ContourPlot } from '@dash-ui/react';
import type { BubblePoint, RadarSeries, BoxSeries, DotPlotItem, HistogramBin, ViolinSeries, ParallelAxis, ParallelSeries, CandlestickBar, BeeswarmSeries, HexbinPoint, ContourPoint } from '@dash-ui/react';
...
export function Wireless() {
  ...
  const filteredAPS = APS.filter(([, , , noise]) => noise >= noiseRange[0] && noise <= noiseRange[1]);
  ...
  return (
    <>
      ...
      <div className="grid">
        ...
        <Card span={6}>
          <h3>AP RF quality</h3>
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>Utilization (x) vs noise floor (y) · bubble = client count</div>
          <BubbleChart
            points={apScatter}
            xRange={[0, 100]}
            yRange={[-102, -90]}
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 8, fontSize: 11, color: '#A4A7B5' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#00C875', display: 'inline-block' }} /> Good</span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#F5A623', display: 'inline-block' }} /> Fair</span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#FF7B7B', display: 'inline-block' }} /> Poor</span>
          </div>
        </Card>

        <Card span={6}>
          <h3>AP performance comparison</h3>
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 4 }}>Signal · Capacity · Clients · Width · Score</div>
          <RadarChart
            series={apRadar}
        ...
        <Card span={6}>
          ...
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>RSSI range per AP (dBm) · box = IQR · line = median</div>
          <BoxPlot
            series={apBoxPlot}
            yRange={[-112, -82]}
            height={150}
          ...
          />
          <div style={{ display: 'flex', gap: 12, marginTop: 8, fontSize: 11, color: '#A4A7B5' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#00C875', display: 'inline-block' }} /> &gt;−95 dBm</span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#F5A623', display: 'inline-block' }} /> −95–−98</span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#FF7B7B', display: 'inline-block' }} /> &lt;−98 dBm</span>
          </div>
        </Card>

        <Card span={6}>
          <h3>Client count per AP</h3>
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>Current week vs previous week</div>
          <DotPlot
            items={apDotPlot}
            min={0}
            max={50}
        ...
        <Card span={6}>
          ...
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>Sample count per signal-strength bin (dBm) · all APs</div>
          <Histogram
            bins={rssiHistogram}
            height={160}
            color="#006FFF"
        ...
        <Card span={6}>
          ...
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>Distribution shape (dBm) · line = median</div>
          <ViolinPlot
            series={apViolin}
            yRange={[-110, -84]}
            height={160}
        ...
        <Card span={6}>
          ...
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>Parallel coordinates: utilisation · SNR · clients · width · uptime</div>
          <ParallelCoordinates
            axes={apParallelAxes}
          ...
          />
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 8, fontSize: 11, color: '#A4A7B5' }}>
        ...
        <Card span={6}>
          ...
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>Signal-to-noise ratio per 3 h interval (dB) · open/close = period start/end</div>
          <CandlestickChart
            bars={snrCandles}
            yRange={[10, 33]}
            height={160}
          ...
          />
          <div style={{ display: 'flex', gap: 12, marginTop: 8, fontSize: 11, color: '#A4A7B5' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 10, height: 10, background: '#00C875', display: 'inline-block' }} /> Improved</span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 10, height: 10, background: '#FF7B7B', display: 'inline-block' }} /> Degraded</span>
          </div>
        </Card>

        <Card span={6}>
          <h3>Client RSSI distribution per AP</h3>
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>Each dot = one client sample · dBm · last hour</div>
          <BeeswarmChart
            series={rssiSwarm}
            yRange={[-110, -80]}
            height={180}
        ...
        <Card span={6}>
          ...
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>2D hexbin · x = RSSI (dBm) · y = SNR (dB) · brightness = sample density</div>
          <HexbinChart
            points={hexbinPts}
            xRange={[-105, -50]}
            yRange={[0, 42]}
        ...
        <Card span={6}>
          ...
          <div style={{ fontSize: 11, color: '#6E7079', marginBottom: 8 }}>Iso-density contours · x = RSSI (dBm) · y = SNR (dB) · dots = samples</div>
          <ContourPlot
            points={contourPts}
            xRange={[-105, -50]}
            yRange={[0, 42]}
