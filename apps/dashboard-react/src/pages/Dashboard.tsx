...
import { Card, Pill, Button, SearchBox, Sparkline, Donut, Pill as P, StatusIndicator, PlusIcon, LineChart, BarChart, WaterfallChart, SankeyDiagram, WaffleChart, StreamGraph, NightingaleChart, DualAxisChart, Banner, AvatarGroup, RankedList } from '@dash-ui/react';
import type { SankeyNode, SankeyLink, StreamSeries, RankedItem } from '@dash-ui/react';
...
const HOURLY_CLIENTS = [2, 1, 1, 1, 2, 5, 18, 45, 67, 73, 68, 55, 58, 62, 65, 71, 78, 82, 75, 60, 42, 28, 15, 7].map(
...
export function Dashboard({ onAdopt }: DashboardProps) {
  ...
  return (
    <>
      {showBanner && (
        <Banner variant="warn" title="Firmware update available" action={{ label: 'Update now', onClick: () => setShowBanner(false) }} onDismiss={() => setShowBanner(false)}>
      ...
      <div className="grid">
        ...
        <Card span={12}>
          <h3>Network Throughput <span className="unit">Last 24 h</span></h3>
          <LineChart
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 8, fontSize: 12 }}>
            {[['Download', '#006FFF'], ['Upload', '#00C8C8']].map(([name, color]) => (
              <span key={name} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ width: 16, height: 2, background: color, borderRadius: 1, display: 'inline-block' }} aria-hidden="true" />
        ...
        <Card span={8}>
          <h3>Traffic by Device Type <span className="unit">Last 24 h · Mbps</span></h3>
          <BarChart
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 8, fontSize: 12 }}>
            {[['Download', '#006FFF'], ['Upload', '#00C8C8']].map(([name, color]) => (
              <span key={name} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ width: 10, height: 10, background: color as string, borderRadius: 2, display: 'inline-block' }} aria-hidden="true" />
                <span style={{ color: '#A4A7B5' }}>{name}</span>
              </span>
            ))}
          </div>
        ...
        <Card span={4}>
          <h3>Client Delta <span className="unit">Mon – Sun</span></h3>
          <WaterfallChart
        ...
        <Card span={8}>
          <h3>Traffic Flow <span className="unit">Last 24 h · Mbps</span></h3>
          <SankeyDiagram
            nodes={TRAFFIC_NODES}
            links={TRAFFIC_LINKS}
            height={180}
        ...
        <Card span={4}>
          <h3>Device Status <span className="unit">12 managed</span></h3>
          <WaffleChart
        ...
        <Card span={8}>
          <h3>Traffic Composition <span className="unit">Last 24 h · Mbps</span></h3>
          <StreamGraph
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 8, fontSize: 12, flexWrap: 'wrap' }}>
            {STREAM_SERIES.map((s) => (
              <span key={s.label} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <span style={{ width: 10, height: 10, background: s.color, borderRadius: 2, display: 'inline-block' }} aria-hidden="true" />
        ...
        <Card span={4}>
          <h3>Client Activity <span className="unit">by hour · avg clients</span></h3>
          <NightingaleChart
        ...
        <Card span={8}>
          <h3>Throughput vs Clients <span className="unit">Last 24 h · Mbps / count</span></h3>
          <DualAxisChart
        ...
        <Card span={8}>
          <h3>Top Clients by Usage <span className="unit">Last 24 h · MB</span></h3>
          <RankedList items={TOP_CLIENTS} unit="MB" ariaLabel="Top clients by data usage in the last 24 hours" />
        </Card>

        <Card span={4}>
          <h3>Top APs by Traffic <span className="unit">Last 24 h · Mbps</span></h3>
          <RankedList items={TOP_APS} unit="Mbps" ariaLabel="Top access points by traffic in the last 24 hours" />
        </Card>
